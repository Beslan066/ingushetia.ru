...
export default function MunicipalityList({ title, items, selected, onActive, getProperties }) {
  if (!items?.length) {
    return null;
  }
          ...
          items.map((settlement) => {
            return (
              <React.Fragment key={ settlement.id }>
                ...
                <MunicipalityDemo id={ settlement.id } title={ settlement.title } image={ settlement.image_main } properties={ getProperties(settlement) } isMobile={ true } isOpened={ selected === settlement.id }/>
              </React.Fragment>
            )
          })
        }
